...
def parse_global_installs(verbose: bool = False):
    ...
    for line in list_result.stdout.split("\n"):
        ...
        if match:
            ...
            # fix lost owners
            if (
                "/" not in match_groups["lib_req"]
                and len(req_entry) > 0
                and req_entry[0].count("/") == 1
            ):
                match_groups["lib_req"] = (
                    req_entry[0].split("/")[0] + "/" + match_groups["lib_req"]
                )
                if int(verbose) >= 1:
                    print("Library Req Adjusted: {}".format(match_groups["lib_req"]))
            if match_groups["lib_req"].count("/") == 1:
                match_groups["lib_owner"] = match_groups["lib_req"].split("/")[0]
            elif match_groups["lib_req"].startswith("https://github.com/"):
                match_groups["lib_owner"] = (
                    match_groups["lib_req"]
                    .replace("https://github.com/", "")
                    .split("/")[0]
                )
            elif match_groups["lib_req"].startswith("Adafruit "):
                match_groups["lib_owner"] = "adafruit"
            else:
                print(f"NO OWNER FOR {line}")
...
for dep_spec in dependencies:
    # print(dep_spec)
    matched_install = []
    if dep_spec.owner is not None:
        matched_install = list(
            filter(
                lambda a_lib: a_lib["lib_name"].lower() == dep_spec.name.lower()
                and a_lib["lib_owner"].lower() == dep_spec.owner.lower(),
                lib_list,
            )
        )
        # print(matched_install)
    else:
        matched_install = list(
            filter(
                lambda a_lib: a_lib["lib_name"].lower() == dep_spec.name.lower(),
    ...
    if len(matched_install) == 0:
        libs_to_install.append(dep_spec)
        continue

    if len(matched_install) > 1:
        raise Exception(
            f"Got more than one installed library match for the spec {dep_spec}!"
        )

    if dep_spec.requirements is not None and dep_spec.requirements.match(
        semantic_version.Version(matched_install[0]["lib_version"])
    ):
        libs_up_to_date.append(dep_spec)
    else:
        libs_to_update.append(dep_spec)

print("Up-To-Date libraries")
print([dep.as_dependency() for dep in libs_up_to_date])
...
with open(libdeps_ini_file, "w+") as out_file:
    ...
    out_file.write(f"; File last updated at {datetime.datetime.now()}\n")
